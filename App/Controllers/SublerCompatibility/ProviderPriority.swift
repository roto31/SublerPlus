...
public struct ProviderPriority: Codable, Sendable {
    ...
    /// Sort MetadataResult arrays by provider priority
    public func sortResults(_ results: [MetadataResult], by sourceExtractor: (MetadataResult) -> String?) -> [MetadataResult] {
        return results.sorted { result1, result2 in
            let source1 = sourceExtractor(result1) ?? ""
            let source2 = sourceExtractor(result2) ?? ""
            let priority1 = priority(for: source1)
            let priority2 = priority(for: source2)
...
/// Extension to sort Subler's MetadataResult by provider priority
extension ProviderPriority {
    /// Sort Subler MetadataResult array by provider source priority
    public func sortSublerResults(_ results: [MetadataResult]) -> [MetadataResult] {
        return results.sorted { result1, result2 in
            // Extract provider name from serviceContentID or other metadata
            // This is a simplified approach - in practice, we'd need to track provider per result
            // For now, we maintain original order if we can't determine provider
            return false
        }
    }
}
